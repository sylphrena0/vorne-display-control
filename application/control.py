...
from flask import Blueprint, Response, flash, render_template, request, session  # web framework imports
...
def index():
    if request.method == "POST":  # if the user hits the submit button
        ...
        try:  # send new message to database and log any errors
            ...
            username = (
                get_db().execute("SELECT * FROM user WHERE id = {}".format(session["user_id"])).fetchone()["username"]
            )  # grab user data to check old password
